...
export default async function BudgetPage({ searchParams }: { searchParams: Promise<{ period?: string }> }) {
  ...
  return <main className="mx-auto max-w-7xl px-6 py-8 sm:py-10 lg:px-10"><div className="mb-8 flex flex-col gap-5 sm:mb-10 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between"><div className="order-1"><p className="label mb-3">Household overview · {label}</p><h1 className="serif text-4xl sm:text-5xl">Budget</h1></div><div className="order-2 sm:order-3 sm:w-full"><FilterBar /></div><div className="order-3 sm:order-2"><ConnectButton /></div></div><section className="grid grid-cols-3 items-center gap-3 border-b hairline py-8 text-center sm:items-end sm:gap-8"><div><p className="label">Total income</p><p className="serif mt-2 text-lg text-forest sm:text-3xl">${totalIncome.toLocaleString()}</p></div><div><p className="label">Total spent</p><p className="serif mt-2 text-3xl sm:text-5xl">${totalSpent.toLocaleString()}</p></div><div><p className="label">Total saved</p><p className="serif mt-2 text-lg text-forest sm:text-3xl">${totalSaved.toLocaleString()}</p></div></section><div className="mt-10 grid gap-10 lg:grid-cols-[1.2fr_.8fr] lg:gap-16"><div><div className="mb-6 flex items-end justify-between"><div><p className="label">Plan vs actual</p><h2 className="serif mt-1 text-2xl">{label}</h2></div><span className="text-xs text-ink/50">Spent / budget</span></div><CategoryTable /><SpendPacing /></div><SpendCharts top={spendBreakdown} detailByBucket={detailByBucket} total={totalSpent + totalSaved} /></div><section className="mt-10 border-b hairline py-10"><div className="mb-6"><p className="label">Money flow</p><h2 className="serif mt-1 text-2xl">{label}</h2></div><SpendSankey total={sankeyTotal} buckets={sankeyBuckets} /></section><div className="mt-12"><Link className="button-quiet inline-block" href="/budget/transactions">View all transactions →</Link></div></main>;
}
