...
export function CategoryTable() {
  ...
  async function loadData() {
    ...
    const [{ data: categoryRows }, { data: expenseRows }] = await Promise.all([
      ...
      supabase.from('transactions').select('id,name,amount,date,category_id,accounts(name,institution)').eq('is_ignored', false).gte('date', start).lte('date', end).order('date', { ascending: false }),
    ]);
