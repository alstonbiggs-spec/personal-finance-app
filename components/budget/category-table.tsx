...
type Expense = { id: string; name: string; amount: number; date: string; category_id: string | null; accounts: ExpenseAccount | ExpenseAccount[] | null };
...
export function CategoryTable() {
  ...
  async function loadData() {
    ...
    const [{ data: categoryRows }, { data: expenseRows }] = await Promise.all([
      ...
      supabase.from('transactions').select('id,name,amount,date,category_id,accounts(name,institution)').eq('is_ignored', false).gte('amount', 0).gte('date', start).lte('date', end).order('date', { ascending: false }),
    ]);
  ...
  const ignoreExpense = async (expense: Expense) => { setExpenses(rows => rows.filter(row => row.id !== expense.id)); const { error } = await supabase.from('transactions').update({ is_ignored: true, updated_at: new Date().toISOString() }).eq('id', expense.id); if (error) setStatus(error.message); };
  ...
  const sortExpenses = (list: Expense[]) => [...list].sort((a, b) => {
    ...
    if (expenseSort.key === 'amount') return (Number(a.amount) - Number(b.amount)) * factor;
