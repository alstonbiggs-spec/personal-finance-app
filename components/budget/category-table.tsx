...
const SORT_LABELS: Record<SortKey, string> = { date: 'Date', amount: 'Amount', name: 'Name' };
...
export function CategoryTable() {
  ...
  const { start, end, months } = resolvePeriod({ period: searchParams.get('period'), from: searchParams.get('from'), to: searchParams.get('to') });
  // Budgets are stored per month; scale them to the length of the selected period.
  const periodBudget = (category: Category) => Number(category.monthly_budget) * months;
  ...
  const toggleExpenseSort = (key: SortKey) => setExpenseSort(prev => prev.key === key ? { key, dir: prev.dir === 'desc' ? 'asc' : 'desc' } : { key, dir: key === 'name' ? 'asc' : 'desc' });
  const sortArrow = (key: SortKey) => expenseSort.key === key ? (expenseSort.dir === 'desc' ? ' ↓' : ' ↑') : '';
  const sortExpenses = (list: Expense[]) => [...list].sort((a, b) => {
    ...
    if (expenseSort.key === 'name') return a.name.localeCompare(b.name, undefined, { sensitivity: 'base' }) * factor;
  ...
  return <div className="overflow-hidden border-t hairline">{status && <p className="border-b border-red-200 bg-red-50 px-4 py-3 text-xs text-red-800">{status}</p>}{groups.map(group => { const rows = categories.filter(category => category.parent_category === dbGroup(group)).sort((a, b) => a.sort_order - b.sort_order); const spent = rows.reduce((sum, row) => sum + (spentByCategory[row.id] ?? 0), 0); const budget = rows.reduce((sum, row) => sum + periodBudget(row), 0); const accent = group === 'INCOME' ? 'text-forest' : 'text-gold'; return <div key={group}>
    ...
    {open.includes(group) && <div className="border-b hairline">{rows.map(category => { const categoryExpenses = expenses.filter(expense => expense.category_id === category.id); const categorySpent = spentByCategory[category.id] ?? 0; const isEditing = editing === category.id; return <div key={category.id} onDragOver={event => { event.preventDefault(); setDragOverId(category.id); }} onDragLeave={() => setDragOverId(current => current === category.id ? null : current)} onDrop={event => { const draggedId = event.dataTransfer.getData('category-id'); if (draggedId) reorder(group, draggedId, category.id); }}><div className={`h-0.5 transition-all ${dragOverId === category.id ? 'mx-4 bg-gold/70 shadow-[0_0_0_3px_rgba(173,138,80,0.12)]' : 'bg-transparent'}`} /><div className="grid grid-cols-[1fr_auto] gap-3 py-3 pl-4 text-left text-sm hover:bg-white/50 sm:gap-4 sm:pl-8"><button onClick={() => setExpanded(expanded === category.id ? null : category.id)} className="text-left"><span draggable={!isEditing} onDragStart={event => { event.stopPropagation(); event.dataTransfer.effectAllowed = 'move'; event.dataTransfer.setData('category-id', category.id); }} onDragEnd={() => setDragOverId(null)} onClick={event => event.stopPropagation()} className="mr-1 hidden cursor-grab text-ink/35 active:cursor-grabbing sm:inline-block">⋮⋮</span><span className={accent}>{expanded === category.id ? '−' : '+'}</span>&nbsp; {category.name}</button><div className="flex items-center gap-2 sm:gap-3"><span className={group === 'INCOME' ? 'text-forest' : ''}>{money(categorySpent)}</span> <span className="text-ink/35">/</span> {isEditing ? <input aria-label={`${category.name} budget`} type="number" value={category.monthly_budget} onChange={e => updateCategory(category, { monthly_budget: Number(e.target.value) })} className="w-20 border-b hairline bg-transparent text-right" /> : <span>{money(periodBudget(category))}</span>}{isEditing && months > 1 && <span className="text-[10px] text-ink/40">/mo</span>}<button onClick={() => setEditing(isEditing ? null : category.id)} className="text-[10px] uppercase tracking-wider text-ink/40">{isEditing ? 'Done' : 'Edit'}</button>{category.name !== 'Other' && <button onClick={() => deleteCategory(category)} className="text-[10px] uppercase tracking-wider text-red-700/60">Delete</button>}</div></div>{isEditing && <div className="flex gap-2 pb-3 pl-12"><input aria-label={`${category.name} name`} value={category.name} onChange={e => updateCategory(category, { name: e.target.value })} className="border-b hairline bg-transparent text-xs" /><span className="text-xs text-ink/40">Changes save automatically</span></div>}{expanded === category.id && <div className="mb-3 ml-6 border-l border-gold/40 pl-3 text-xs text-ink/60 sm:ml-12 sm:pl-4">{categoryExpenses.length > 1 && <div className="flex items-center gap-4 border-b hairline py-2 text-[10px] uppercase tracking-wider sm:hidden"><span className="text-ink/40">Sort</span>{(['date', 'amount', 'name'] as SortKey[]).map(key => <button key={key} onClick={() => toggleExpenseSort(key)} className={`py-1 ${expenseSort.key === key ? 'text-gold' : ''}`}>{SORT_LABELS[key]}{sortArrow(key)}</button>)}</div>}<div className="hidden border-b hairline py-2 uppercase tracking-wider sm:grid sm:grid-cols-[1fr_90px_110px_150px_60px] sm:items-center"><button onClick={() => toggleExpenseSort('name')} className={`text-left hover:text-gold ${expenseSort.key === 'name' ? 'text-gold' : ''}`}>Expense{sortArrow('name')}</button><button onClick={() => toggleExpenseSort('amount')} className={`text-left hover:text-gold ${expenseSort.key === 'amount' ? 'text-gold' : ''}`}>Amount{sortArrow('amount')}</button><button onClick={() => toggleExpenseSort('date')} className={`text-left hover:text-gold ${expenseSort.key === 'date' ? 'text-gold' : ''}`}>Date{sortArrow('date')}</button><span>Assign category</span><span></span></div>{categoryExpenses.length === 0 ? <p className="py-3">No expenses in this category.</p> : sortExpenses(categoryExpenses).map(expense => { const amountEdited = expense.original_amount !== null && expense.original_amount !== undefined && Number(expense.original_amount) !== Number(expense.amount); const dateEdited = expense.original_date !== null && expense.original_date !== undefined && expense.original_date !== expense.date; return <div key={expense.id} className="group border-b hairline py-3"><div className="flex flex-col gap-2 sm:grid sm:grid-cols-[1fr_90px_110px_150px_60px] sm:items-center sm:gap-0"><div className="flex items-center justify-between gap-3 sm:contents">{editingExpenseId === expense.id ? <input aria-label="Expense name" autoFocus value={editingExpenseName} onChange={e => setEditingExpenseName(e.target.value)} onBlur={() => commitExpenseName(expense)} onKeyDown={e => { if (e.key === 'Enter') commitExpenseName(expense); if (e.key === 'Escape') setEditingExpenseId(null); }} className="border-b hairline bg-transparent" /> : <button onClick={() => startEditingExpense(expense)} className="text-left hover:underline">{expense.name}</button>}<span className="flex items-center gap-1.5">{editingExpenseAmountId === expense.id ? <input aria-label="Expense amount" autoFocus type="number" step="0.01" min="0" value={editingExpenseAmountValue} onChange={e => setEditingExpenseAmountValue(e.target.value)} onBlur={() => commitExpenseAmount(expense)} onKeyDown={e => { if (e.key === 'Enter') commitExpenseAmount(expense); if (e.key === 'Escape') setEditingExpenseAmountId(null); }} className="w-16 border-b hairline bg-transparent" /> : <button onClick={() => startEditingExpenseAmount(expense)} className={`hover:underline ${group === 'INCOME' ? 'font-medium text-forest' : ''} ${amountEdited ? 'text-gold' : ''}`}>{money(Math.abs(Number(expense.amount)))}</button>}{amountEdited && editingExpenseAmountId !== expense.id && <span className="group/edit relative inline-block"><span className="block h-1.5 w-1.5 rounded-full bg-gold" /><span className="invisible absolute left-1/2 top-full z-20 mt-1 -translate-x-1/2 whitespace-nowrap rounded-sm border hairline bg-white px-2 py-1 text-[10px] text-ink/60 opacity-0 shadow-lg transition duration-150 group-hover/edit:visible group-hover/edit:opacity-100">Originally {money(Math.abs(Number(expense.original_amount)))}</span></span>}</span></div><div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs sm:contents"><span className="flex items-center gap-1.5">{editingExpenseDateId === expense.id ? <input aria-label="Expense date" autoFocus type="date" value={editingExpenseDateValue} onChange={e => setEditingExpenseDateValue(e.target.value)} onBlur={() => commitExpenseDate(expense)} onKeyDown={e => { if (e.key === 'Enter') commitExpenseDate(expense); if (e.key === 'Escape') setEditingExpenseDateId(null); }} className="border-b hairline bg-transparent text-xs" /> : <button onClick={() => startEditingExpenseDate(expense)} className={`text-xs hover:underline ${dateEdited ? 'text-gold' : 'text-ink/60'}`}>{formatExpenseDate(expense.date)}</button>}{dateEdited && editingExpenseDateId !== expense.id && <span className="group/edit relative inline-block"><span className="block h-1.5 w-1.5 rounded-full bg-gold" /><span className="invisible absolute left-1/2 top-full z-20 mt-1 -translate-x-1/2 whitespace-nowrap rounded-sm border hairline bg-white px-2 py-1 text-[10px] text-ink/60 opacity-0 shadow-lg transition duration-150 group-hover/edit:visible group-hover/edit:opacity-100">Originally {formatExpenseDate(expense.original_date as string)}</span></span>}</span><select value={expense.category_id ?? ''} onChange={e => reassign(expense, e.target.value)} className="bg-transparent text-xs">{categories.filter(row => row.parent_category === dbGroup(group)).map(row => <option key={row.id} value={row.id}>{row.name}</option>)}</select><button onClick={() => ignoreExpense(expense)} className="text-right text-[10px] uppercase tracking-wider text-ink/40 hover:text-red-700">Ignore</button></div></div><div className="pt-1 text-[10px] text-ink/40 sm:max-h-0 sm:overflow-hidden sm:pt-0 sm:transition-all sm:duration-200 sm:group-hover:max-h-6 sm:group-hover:pt-1">{expenseAccountLabel(expense)}</div></div>; })}</div>}</div>; })}<button onClick={() => addCategory(group)} className={`mb-4 ml-6 text-[10px] uppercase tracking-[.16em] sm:ml-12 ${accent}`}>+ Add category</button><p className="mb-4 ml-6 hidden text-[10px] text-ink/40 sm:ml-12 sm:block">Drag the ⋮⋮ handle to reorder categories. The gold line shows the drop position.</p></div>}
  </div>; })}</div>;
}
