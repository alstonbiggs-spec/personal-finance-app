...
type Account = { id: string; name: string; institution: string; account_type: string; lastSyncedAt: string | null; syncError: string | null; needsReauth: boolean };
...
export function ConnectButton() {
  ...
  return <div className="text-right"><button onClick={() => open()} disabled={loading || !ready} className="button-quiet">{loading ? 'Loading…' : 'Connect an account'}</button>{accounts.length > 0 && <div className="mt-4 text-left"><p className="label">Connected accounts</p><div className="mt-2 space-y-1">{accounts.map(account => <div key={account.id}><p className="text-xs text-ink/60">{account.institution} · {account.name}</p>{account.needsReauth ? <p className="text-xs text-red-500">Needs to be reconnected — click &ldquo;Connect an account&rdquo; and relink {account.institution}.</p> : account.syncError ? <p className="text-xs text-red-500">{account.syncError}</p> : account.lastSyncedAt ? <p className="text-xs text-ink/40">Last synced {new Date(account.lastSyncedAt).toLocaleString()}</p> : <p className="text-xs text-ink/40">Not yet synced</p>}</div>)}</div><button onClick={syncTransactions} disabled={syncing} className="mt-3 text-xs uppercase tracking-wider text-gold">{syncing ? 'Syncing…' : 'Sync transactions'}</button></div>}{message && <p className="mt-2 max-w-xs text-xs text-ink/55">{message}</p>}</div>;
}
