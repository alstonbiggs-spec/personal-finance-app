...
export function ConnectButton() {
  ...
  useEffect(() => { fetch('/api/plaid/create-link-token', { method: 'POST' }).then(async response => { const data = await response.json(); if (!response.ok) throw new Error(data.error); setToken(data.link_token); }).catch(error => setMessage(error.message || 'Plaid is not configured.')).finally(() => { setLoading(false); void loadAccounts(); }); }, []);
  const onSuccess = useCallback<PlaidLinkOnSuccess>(async (public_token, metadata) => { setMessage('Finishing connection…'); const response = await fetch('/api/plaid/exchange-public-token', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ public_token, metadata }) }); const data = await response.json(); setMessage(data.error ?? data.message ?? 'Account connected.'); if (response.ok) await loadAccounts(); }, []);
