...
export function ConnectButton() {
  ...
  // Pull fresh transactions automatically so the dashboard never shows stale data without the
  // user having to remember to hit "Sync transactions" — throttled per browser so repeated
  // page visits within a few minutes don't hammer Plaid.
  useEffect(() => {
    const AUTO_SYNC_THROTTLE_MS = 5 * 60 * 1000;
    const last = Number(localStorage.getItem('plaid-last-auto-sync') ?? 0);
    if (Date.now() - last < AUTO_SYNC_THROTTLE_MS) return;
    localStorage.setItem('plaid-last-auto-sync', String(Date.now()));
